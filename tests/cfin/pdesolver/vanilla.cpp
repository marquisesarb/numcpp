#include <iostream>
#include <cassert>
#include <numcpp/cfin/pdesolver/vanilla.hpp>
...
void europeanFlatSigma(bool useCrankNicolson) {
    ...
    while (MM < 600) {
        ...
        numcpp::cfin::PDESolverLogVanillaEngine pdeEngine(S,K,T,mu,r,sigma,false,false,N,MM, useCrankNicolson,10);
...
void europeanLocalVolatility(bool useCrankNicolson) {
    ...
    numcpp::cfin::PDESolverLogVanillaEngine pdeEngine(S,K,T,true,false,driftFun,localVolFun,discountRateFun,N,M, useCrankNicolson,5);

    for (size_t i = 0; i<2*M+1; i++) {

        std::cout << "Spot Price: " << pdeEngine.spotPriceVector(i) <<", Option Price: " << pdeEngine.optionValueMatrix.col(0)(i) << std::endl;
    }
}
